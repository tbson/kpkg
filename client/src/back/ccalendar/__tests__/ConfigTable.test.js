...
import {Row, CCalendarTable} from '../tables/CCalendarTable';
import LoadingLabel from 'src/utils/components/LoadingLabel';
import Tools from 'src/utils/helpers/Tools';

Enzyme.ccalendarure({adapter: new Adapter()});
...
describe('CCalendarTable Row component', () => {
    let wrapper;
    const props = {
        _key: 0,
        data: {
...
describe('CCalendarTable component', () => {
    ...
    it('Get list', done => {
        ...
        const wrapper = shallow(<CCalendarTable {...props} />);
        ...
        setTimeout(() => {
            ...
            expect(props.action.mock.calls[0][1]).toEqual({
                data: props.ccalendarState.list,
                pages: props.ccalendarState.pages,
            });
    ...
    it('Add', done => {
        ...
        const toggleModalSpy = jest.spyOn(CCalendarTable.prototype, 'toggleModal');
    ...
    describe('Bulk remove', () => {
        ...
        it('No check', done => {
            const handleRemoveSpy = jest.spyOn(CCalendarTable.prototype, 'handleRemove').mockImplementation(() => null);
        ...
        it('Check 1', done => {
            props.ccalendarState.list[0].checked = true;
        ...
        it('Check 3', done => {
            props.ccalendarState.list[0].checked = true;
            props.ccalendarState.list[1].checked = true;
            props.ccalendarState.list[2].checked = true;
    ...
    describe('CCalendarTable methods', () => {
        ...
        it('list', async () => {
            ...
            const setInitDataSpy = jest.spyOn(CCalendarTable.prototype, 'setInitData');
            ...
            expect(apiCallSpy.mock.calls[0][0]).toEqual('about:///api/v1/ccalendar/');
            ...
            // Default URL
            expect(apiCallSpy).toHaveBeenCalled();
            expect(apiCallSpy.mock.calls[1][0]).toEqual('about:///api/v1/ccalendar/');
        ...
        describe('toggleModal', () => {
            it('Not defined modalName or null ID', async () => {
                ...
                let wrapper = shallow(<CCalendarTable {...props} />);
        ...
        it('handleSubmit', async () => {
            ...
            const toggleModal = jest.spyOn(CCalendarTable.prototype, 'toggleModal')
                .mockImplementation(
                    () => ({}),
                );
            jest.spyOn(CCalendarTable.prototype, 'componentDidMount').mockImplementation(() => {});
            ...
            const handleAddSuccess = jest.spyOn(CCalendarTable.prototype, 'handleAdd')
                .mockImplementation(
                    async params => null
                );
            const handleEditSuccess = jest.spyOn(CCalendarTable.prototype, 'handleEdit')
                .mockImplementation(
                    async params => null
                );
